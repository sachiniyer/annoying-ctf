...
class Terminal extends CoreTerminal_1.CoreTerminal {
    ...
    _handleColorEvent(event) {
        ...
        for (const req of event) {
            ...
            switch (req.type) {
                ...
                case 1:
                    if (acc === 'ansi') {
                        this._themeService.modifyColors(colors => colors.ansi[req.index] = Color_1.rgba.toColor(...req.color));
                    }
                    else {
                        const narrowedAcc = acc;
                        this._themeService.modifyColors(colors => colors[narrowedAcc] = Color_1.rgba.toColor(...req.color));
                    }
                    break;
                case 2:
    ...
    _handleScreenReaderModeOptionChange(value) {
        ...
        if (value) {
            if (!this._accessibilityManager && this._renderService) {
                this._accessibilityManager = this._instantiationService.createInstance(AccessibilityManager_1.AccessibilityManager, this);
            }
        }
        else {
            (_a = this._accessibilityManager) === null || _a === void 0 ? void 0 : _a.dispose();
    ...
    open(parent) {
        ...
        this.viewport = this._instantiationService.createInstance(Viewport_1.Viewport, (amount) => this.scrollLines(amount, true, 1), this._viewportElement, this._viewportScrollArea);
    ...
    _createRenderer() {
        return this._instantiationService.createInstance(DomRenderer_1.DomRenderer, this.element, this.screenElement, this._viewportElement, this.linkifier2);
    }
    bindMouse() {
        const self = this;
